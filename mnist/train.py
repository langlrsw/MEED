...
hidden_dims = 250
epochs = 5
k =25# Number of selected words by MEED.
PART_SIZE = 125
###########################################
###############Load data###################
###########################################
...
def load_data():
    ...
    data_path = 'data/mnist_aligned.npz'
    npz_data = np.load(data_path)
    x_train, y_train, x_val, y_val = npz_data['x_train'], npz_data['y_train'], npz_data['x_test'], npz_data[
        'y_test']
    
    idx = np.where(((y_train == 8)+(y_train == 3))>0)[0]
    x_train = x_train[idx]
    y_train = y_train[idx]
    idx = np.where(((y_val == 8)+(y_val == 3))>0)[0]
    x_val = x_val[idx]
    y_val = y_val[idx]
...
def MEED(train = True): 
    ...
    with session as sess:
        ...
        pred_train_one_hot = to_categorical(np.argmax(pred_train,axis=1),num_classes)

        idx = np.arange(x_train.shape[0])
        num_batch = int(np.ceil(x_train.shape[0]/float(256)))
        selection_train = []
        for i_batch in range(num_batch):
            ...
            selection_train_i = sess.run(grad_sum, feed_dict={x_placeholder: x_train[idx_i], y_placeholder: pred_train_one_hot[idx_i]})
            selection_train.append(selection_train_i)
        selection_train = np.vstack(selection_train)
...
#                     selection_prior = selection_prior[:,:,:,np.newaxis]
                    ...
                    if flag_with_y:
                        model_pos.train_on_batch([x_batch,y_batch,mean_x,selection_prior,epoch_cur],[y_batch,y_batch])
                    else:
                        model_pos.train_on_batch([x_batch,mean_x,selection_prior,epoch_cur],[y_batch,y_batch])

                    if flag_with_y:
                        model_neg.train_on_batch([x_batch,y_batch,mean_x,selection_prior,epoch_cur],[y_batch,1.-y_batch,epoch_cur])
                    else:
                        model_neg.train_on_batch([x_batch,mean_x,selection_prior,epoch_cur],[y_batch,1.-y_batch,epoch_cur])
