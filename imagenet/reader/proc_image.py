...
def reverse_tile(tile,grid_size,ori_image_size):
    image_size = grid_size * tile[0].shape[0]
    ...
    for n in range(n_grids):
        y = int(n /  grid_size)
        x = n %  grid_size
        img[x * w:(x + 1) * w,  y * w:(y + 1) * w,:] = tile[n]
    img = resize(img, ori_image_size)
#     tile = self._augment_tile(tile)
    return img
 
def jigsaw(img,grid_size,order,permutations):
       
    n_grids = grid_size ** 2
    tiles = [None] * n_grids
    for n in range(n_grids):
        tiles[n] = get_tile(img, n,grid_size)
  
    if order == 0:
        return img
    else:
        data = [tiles[permutations[order - 1][t]] for t in range(n_grids)]
        return reverse_tile(data,grid_size,img.shape[0])
...
def rotation_numpy(image, angle=90):
    if angle == 90:
